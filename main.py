...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("fuel_stations.log", mode='w'),  # Overwrite log file each run
...
def main():
    ...
    try:
        ...
        logger.info(f"✓ Road network downloaded: {len(G_road.nodes):,} nodes, {len(G_road.edges):,} edges")
        ...
        if len(stations) > 100:
            ...
            logger.info(f"⚠ Sampled {len(stations)} stations from {original_count} for analysis performance")

        logger.info(f"✓ Fuel stations loaded: {len(stations)} stations")
        
        if len(stations) < 10:
            logger.error(f"✗ Insufficient fuel stations: {len(stations)} < 10 minimum required")
            return
        ...
        logger.info(f"✓ Edge filtering complete: removed {removed_edges} edges ({100*removed_edges/initial_edges:.1f}%)")
        ...
        logger.info(f"✓ Optimized filtered graph: {G_fareness_removed.vcount()} nodes, {G_fareness_removed.ecount()} edges")
        ...
        step_start = log_step_start("9.2", "Saving Voronoi diagram for random comparison graph")
        ...
        logger.info("")
        logger.info("Note: All Voronoi diagrams use consistent clipping for fair comparison")
