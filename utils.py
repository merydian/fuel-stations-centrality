...
class Utils:
    ...
    def get_gas_stations_from_graph(self, G):
        ...
        tags = {"amenity": "fuel"}
        logger.info(f"Querying gas stations from PBF file: {self.config.LOCAL_PBF_PATH}")
        ...
        if self.config.MAX_STATIONS:
            ...
            logger.info(f"Limited gas stations from {original_count} to {len(gas_stations)} (MAX_STATIONS={self.config.MAX_STATIONS})")
        ...
        logger.debug(f"Filtering stations within {self.config.STATIONS_MAX_RADIUS}m of nearest network node")
        ...
        gas_stations = gas_stations[gas_stations["node_dist"] <= self.config.STATIONS_MAX_RADIUS]
    ...
    def igraph_edges_to_gpkg(self, g, name):
        ...
        output_path = f"{self.config.OUTPUT_DIR}/{name}_{self.config.PLACE.lower()}_edges.gpkg"
    ...
    def generate_centrality_table(self, graphs_dict):
        ...
        output_file = f"{self.config.OUTPUT_DIR}/centrality_table{self.config.PLACE.lower()}.tex"
    ...
    def generate_graph_info_table(self, graphs_dict):
        ...
        # Save LaTeX table to file
        output_file = f"{self.config.OUTPUT_DIR}/graph_info_{self.config.PLACE.lower()}.tex"
        with open(output_file, "w") as f:
            f.write(latex_table)
