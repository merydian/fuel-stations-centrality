...
if Config.SAMPLE_NODES is not None:
    ...
    if Config.SAMPLE_NODES < original_nodes:
        logger.info(f"Sampling road network: {original_nodes:,} → {Config.SAMPLE_NODES:,} nodes")
        ...
        logger.info(f"✓ Graph sampling completed:")
        logger.info(f"  • Nodes: {original_nodes:,} → {sampled_nodes_count:,} ({100 * sampled_nodes_count / original_nodes:.1f}%)")
        logger.info(f"  • Edges: {original_edges:,} → {sampled_edges_count:,} ({100 * sampled_edges_count / original_edges:.1f}%)")
        
        # Verify the sampled graph still has valid coordinates
        if sampled_nodes_count > 0:
            sample_coords = [(data['x'], data['y']) for node, data in list(G_road.nodes(data=True))[:3]]
            if sample_coords:
                x_vals = [x for x, y in sample_coords]
                y_vals = [y for x, y in sample_coords]
                logger.debug(f"Sample coordinates after sampling: X={x_vals}, Y={y_vals}")
    else:
        logger.info(f"SAMPLE_NODES ({Config.SAMPLE_NODES:,}) >= graph size ({original_nodes:,}), no sampling needed")
else:
